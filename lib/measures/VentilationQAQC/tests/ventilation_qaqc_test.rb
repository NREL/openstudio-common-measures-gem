...
class VentilationQAQCTest < Minitest::Test
  def model_in_path_default
    return "#{File.dirname(__FILE__)}/small_office.osm"
  end

  def epw_path_default
    # make sure we have a weather data location
    epw = File.expand_path("#{File.dirname(__FILE__)}/USA_IL_Chicago-OHare.Intl.AP.725300_TMY3.epw")
  ...
  def model_out_path(test_name)
    return "#{run_dir(test_name)}/small_office.osm"
  end
